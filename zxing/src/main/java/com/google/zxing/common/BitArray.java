...
public final class BitArray implements Cloneable {
  ...
  private void ensureCapacity(int size) {
    if (size > bits.length * 32) {
      int[] newBits = makeArray(size);// return int[(31+size)/32]
      System.arraycopy(bits, 0, newBits, 0, bits.length);
      this.bits = newBits;// newBits = (bits,0,...,0)
    }
  }
   ...
   */
  public boolean get(int i) {// i番目のbitが'1'であるか 1ならtrue
   ...
   *                corresponds to bit i, the next-least-significant to i+1, and
   *                so on.
   */
  ...
  public void appendBit(boolean bit) {// 下位ビット11101上位ビット
    ensureCapacity(size + 1);
    if (bit) {
      bits[size / 32] |= 1 << (size & 0x1F);// |=:論理和代入, x<<y:xをyビット左にシフト
    }
    size++;
  }
   ...
   * most-significant to
   * least-significant. For example, appending 6 bits from 0x000001E will append
   * the bits
