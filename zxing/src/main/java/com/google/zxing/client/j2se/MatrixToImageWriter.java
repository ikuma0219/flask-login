...
public final class MatrixToImageWriter {
  ...
  public static BufferedImage toBufferedImagekai(BitMatrix[] matrix) {
    return toBufferedImagekai(matrix, DEFAULT_CONFIG);
  }
  ...
  public static BufferedImage toBufferedImage(BitMatrix matrix, MatrixToImageConfig config) {
    ...
    for (int y = 0; y < height; y++) {
      ...
      image.setRGB(0, y, width, 1, rowPixels, 0, width);// (int startX, int startY, int w, int h, int[] rgbArray, int
                                                        // offset, int scansize)
    }
    return image;
  }

  public static BufferedImage toBufferedImagekai(BitMatrix[] matrix, MatrixToImageConfig config) {
    ...
    for (int y = 0; y < height; y++) {
      row = matrix[0].getRow(y, row);
      rowb = matrix[1].getRow(y, rowb);
      for (int x = 0; x < width; x++) {

        // if(x%3==0)
        // rowPixels[x] = 0xFFFFFFFF;
        // if(x%3==1)
        // rowPixels[x] = 0xFF000000;
        // if(x%3==2)
        // rowPixels[x] = 0xFFFF0000;
        if (!row.get(x) && !rowb.get(x))
          rowPixels[x] = 0xFFFFFFFF;
        if (row.get(x) && !rowb.get(x))
          rowPixels[x] = 0xFFFF0000;
        if (row.get(x) && rowb.get(x))
  ...
  /**
   * As {@link #writeToPath(BitMatrix, String, Path)}, but allows customization of
   * the output.
   *
   * @param matrix {@link BitMatrix} to write
   * @param format image format
